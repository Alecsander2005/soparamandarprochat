...
def set_father_entity(entity, father_uuid):
    try:
        if father_uuid and father_uuid != 'None':
            uuid_obj = uuid.UUID(father_uuid)
            father_entity = models.Entity.objects.get(uuid_portal=uuid_obj)
...
def get_all_child_entity_ids(entity_id):
    """
    Obtém todos os IDs de entidades filhas de uma entidade específica, recursivamente.
    """
    child_ids = list(models.Entity.objects.filter(dad_id=entity_id).values_list('id', flat=True))
    for child_id in child_ids[:]:
        child_ids.extend(get_all_child_entity_ids(child_id))
    return child_ids